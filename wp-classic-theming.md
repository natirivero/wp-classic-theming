...
**Color palette syntax:** Each palette entry's `color` value may be standard hex (`#1a2b3c`) or explicit `rgb()` / `rgba()` notation. Use whichever the design file provides — both emit valid `--wp--preset--color--{slug}` variables.
...
GitHub Theme URI: wilnau-design/{slug}
GitHub Branch: main
*/
```
...
- **Layout engine**: Default to CSS Grid and Flexbox for structure. Do not use floats or positioning as layout fallbacks.
- **Syntax & naming**: Use native CSS nesting with semantic, descriptive class names. Do not use BEM (`block__element--modifier`).
- **Breakpoints**: Tablet `@media (max-width: 1024px)`. Mobile `@media (max-width: 600px)`.
- **Section alignment**: First-level `<section>` wrappers (`alignfull`) must not set horizontal padding. Center content in an inner container with `max-width` and `margin: 0 auto` (e.g. `.alignwide`).
